...
def normalize_city_name_with_y_variation(city_name):
    ...
    city_name = re.sub(r'י{2,}', 'י', city_name)
    city_name = re.sub(r'י', '', city_name)
    return city_name
...
def load_data_from_file(file_path='data.csv'):
    ...
    for index, row in original_data.iterrows():
        ...
        # Directly assign latitude and longitude (assuming they exist)
        city_data_dict[city_name] = {
            'Latitude': row['Latitude'],
            'Longitude': row['Longitude'],
            'Population': population,
...
# Ensure that the app binds to the correct port for deployment on Render or similar platforms
